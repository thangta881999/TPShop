...
public class SanPhamDAO implements ISanPham {
    ...
    public List<SanPhamDTO> search(String keyword, int offset, int limit) {
        ...
        org.hibernate.search.FullTextQuery hibQuery = fullTextSession.createFullTextQuery(query, SanPham.class);
        List<SanPham> results = new ArrayList<SanPham>();
        if (offset >= 0) {
            results = hibQuery.setFirstResult(offset).setMaxResults(limit).list();
        } else {
